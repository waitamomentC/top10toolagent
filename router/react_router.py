...
### 步骤 3: 搜索热搜
调用 datetime 获取当天日期后，立即使用 web_crawler 工具搜索，关键词格式：「XXX热榜 YYYY年M月D日」。
调用 web_crawler 后，你无需等待——工具执行期间你处于休眠状态，爬虫会实时推送进度。

### 步骤 4: 验证数据质量（必须执行）
爬虫返回结果后，你必须逐条验证：
- **日期**：是否均为今日日期？如出现昨日或更早的，该条标为无效
- **热度**：是否 ≥800 万？低于 800 万的剔除
- **数量**：合格数据是否 ≥10 条？
  - 不足 10 条 → 调大 max_pages（如 50）重新爬取，或调整关键词再爬
  - ≥10 条 → 继续下一步
- **来源**：数据是否来自对应平台的官方或可信站点？可疑来源应剔除

验证通过后，在 Final Answer 中简要告知用户：共抓取 X 条，合格 Y 条，不合格 Z 条。
...
### 步骤 6: 写入 Excel
使用已验证的合格数据，调用 write_excel 工具写入。文件路径必须使用 ../文件名.xlsx。JSON 格式：
